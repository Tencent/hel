...
class LRUCache<K extends any, V extends any> {
  constructor(options = {}) {
    const { max, ttl, updateAgeOnGet, allowStale, dispose, noDisposeOnSet, maxSize, sizeCalculation } = options;
  ...
  initializeTTLTracking() {
    ...
    this.isStale = (index) => {
      return this.ttls[index] !== 0 && this.starts[index] !== 0 && perf.now() - this.starts[index] > this.ttls[index];
    };
  }
  updateItemAge(index) {}
  ...
  *values() {
  ...
  set(k: K, v: V, { ttl = this.ttl, noDisposeOnSet = this.noDisposeOnSet, size = 0, sizeCalculation = this.sizeCalculation } = {}) {
  ...
  get(k: K, { allowStale = this.allowStale, updateAgeOnGet = this.updateAgeOnGet } = {}): V {
    const index = this.keyMap.get(k);
    if (index !== undefined) {
