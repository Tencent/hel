...
以下内容将使用[hel-tpl-remote-lib](https://www.to-be-added.com/coming-soon)作为示例
### 懒加载远程模块
...
<button onClick={async function(){
  ...
  const lib = await helMicro.preFetchLib(
    'hel-tpl-remote-lib', { versionId:'2.0.0' },
  );
  alert(lib.num.random(22));
}}>固定返回10000</button>
```
...
<button onClick={async function(){
  const helMicro = await import('hel-micro');
  const lib = await helMicro.preFetchLib(
    'hel-tpl-remote-lib', { versionId:'2.0.1' },
  );
  alert(lib.num.random(22));
}}>固定返回888</button>
```
...
- [模块开发-js库](/docs/tutorial/helmod-dev)
- [模块开发-react组件](/docs/tutorial/helmod-dev-react)
- [模块开发-vue组件](/docs/tutorial/helmod-dev-vue)
